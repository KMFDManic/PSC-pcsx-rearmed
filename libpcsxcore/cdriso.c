...
static int uncompressPSC(void *out, unsigned long *out_size, void *in, unsigned long in_size)
{
	static z_stream z;
	int ret = 0;
...
static int cdread_compressed(FILE *f, unsigned int base, void *dest, int sector)
{
	...
	if (is_compressed) {
		...
		ret = uncompressPSC(compr_img->buff_raw[0], &cdbuffer_size, compr_img->buff_compressed, size);
		if (ret != 0) {
