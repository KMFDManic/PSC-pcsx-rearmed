...
int uncompressPSC(void *out, unsigned long *out_size, void *in, unsigned long in_size)
{
	static z_stream z;
	int ret = 0;
...
static long CDRreadTrack(unsigned char *time)
{
	...
	case CDRC_ZLIB:
		...
		break;
	case CDRC_ZLIB2:
		ret = uncompressPSC(cdbuffer->raw[0], &cdbuffer_size, cdbuffer->compressed, size);
		break;
	case CDRC_BZ:
