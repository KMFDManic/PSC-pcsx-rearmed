 ...
 * (C) Artur "screemer" Jakubowicz, 2019 - fixes for PS Classic AutoBleem - alternative sdl input
 *
...
static int func_SDLK_TO_MY_SDLK(int argSDLK) {
  ...
  do {
    if(SDLK_to_AB_SDLK_ARRAY[i].m_SDLK == argSDLK) {
      break;
    }
    i++;
  } while(SDLK_to_AB_SDLK_ARRAY[i].m_SDLK != SDLK_LAST);
  return SDLK_to_AB_SDLK_ARRAY[i].m_MySDLK;
}

// There was some Japanese comment from Sony guy here :)
static const char * const in_ableem_keys[SDLK_LAST] = {
...
static void in_ableem_probe(const in_drv_t *drv)
{
	const struct in_pdata *pdata = drv->pdata;
	const char * const * key_names = in_ableem_keys;
	struct in_ableem_state *state;
	SDL_Joystick *joy;
	int i, joycount;
	char name[256];
	...
	if (state == NULL) {
		fprintf(stderr, "in_ableem: OOM\n");
		return;
	}

	state->drv = drv;
	in_register(IN_ABLEEM_PREFIX "keys", -1, state, SDLK_LAST,
		key_names, 0);
	...
	for (i = 0; i < joycount; i++) {
		...
		if(fd1 != -1 && fd0 == -1) {
			snprintf(name, sizeof(name), IN_ABLEEM_PREFIX "%s [%d]", SDL_JoystickName(joy), 1);
		} else {
			snprintf(name, sizeof(name), IN_ABLEEM_PREFIX "%s", SDL_JoystickName(joy));
		}
		
#else
		snprintf(name, sizeof(name), IN_ABLEEM_PREFIX "%s", SDL_JoystickName(i));
#endif
...
static void in_ableem_free(void *drv_data)
{
	struct in_ableem_state *state = drv_data;
...
in_ableem_get_key_names(const in_drv_t *drv, int *count)
{
	const struct in_pdata *pdata = drv->pdata;
	*count = SDLK_LAST;
...
static int handle_event(struct in_ableem_state *state, SDL_Event *event,
...
static int handle_joy_event(struct in_ableem_state *state, SDL_Event *event,
	int *kc_out, int *down_out)
{
...
#if SDL_MAJOR_VERSION == 2
static void ControlJoysticEscape(struct in_ableem_state *arg_pState,
...
static int collect_events(struct in_ableem_state *state, int *one_kc, int *one_down)
{
	SDL_Event events[4];
#ifdef CONFIG_DEBUG
	static int cnt=0;
...
static int in_ableem_update(void *drv_data, const int *binds, int *result)
{
	struct in_ableem_state *state = drv_data;
	keybits_t mask;
	int i, sym, bit, b;
...
static int in_ableem_update_keycode(void *drv_data, int *is_down)
{
	struct in_ableem_state *state = drv_data;
...
static int in_ableem_menu_translate(void *drv_data, int keycode, char *charcode)
{
	struct in_ableem_state *state = drv_data;
...
static const in_drv_t in_ableem_drv = {
	.prefix         = IN_ABLEEM_PREFIX,
	.probe          = in_ableem_probe,
	.free           = in_ableem_free,
	.get_key_names  = in_ableem_get_key_names,
	.update         = in_ableem_update,
	.update_keycode = in_ableem_update_keycode,
	.menu_translate = in_ableem_menu_translate,
};

int in_ableem_init(const struct in_pdata *pdata, void (*handler)(void *event))
{
	if (!pdata) {
