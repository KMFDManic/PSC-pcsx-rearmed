...
#ifndef NO_FRONTEND
    snprintf(Config.PatchesDir, sizeof(Config.PatchesDir), "."
            PATCHES_DIR);
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-psxout")) psxout = 1;
            ...
            aspect_ratio = atoi(argv[++i]);
            if (aspect_ratio < ASPECT_4_3 || aspect_ratio > ASPECT_16_9) {
    ...
    sprintf(changedisc_message, "%s%s%s%s", LANG_IMG_DIR, "msg_current_inserted_disc_info_",
            lang_list[language - 1],
            ".png");
    sprintf(solodisc_message, "%s%s%s%s", LANG_IMG_DIR, "msg_cant_change_discs_this_game_", lang_list[language - 1],
            ".png");
    sprintf(nottime_message, "%s%s%s%s", LANG_IMG_DIR, "msg_cant_change_discs_now_", lang_list[language - 1],
            ".png");
